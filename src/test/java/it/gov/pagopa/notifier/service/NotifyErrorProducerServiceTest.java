...
@TestPropertySource(properties = {
 ...
 class NotifyErrorProducerServiceTest {
    ...
    void enqueueNotify_OK(){
        notifyErrorProducerService.enqueueNotify(MESSAGE_DTO,TPP_DTO.getTppId(), RETRY).block();
        Mockito.verify(notifyErrorProducer,times(1)).scheduleMessage(any());
    }

    @Test
    void enqueueNotify_KO(){
        notifyErrorProducerService.enqueueNotify(MESSAGE_DTO,TPP_DTO.getTppId(), RETRY_KO).block();
